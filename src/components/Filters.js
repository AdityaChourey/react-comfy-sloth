...
const Filters = () => {
  const {
    filters:{text,category,company,color,min_price,
      max_price,price,shipping},updateFilters,clearFilters,all_products
  } = useFilterContext()
    ...
    <div className="content">
      <form onSubmit={(e)=>e.preventDefault()}>
        ...
        <div className="form-control">
          ...
          className='search-input' value={text} onChange={updateFilters} />
        </div>
        {/* categories */}
        <div className="form-control">
          <h5>category</h5>
        ...
        <div className="form-control">
          ...
          <select name="company" value={company} onChange={updateFilters} className='company'>
            {companies.map((co,index)=>{
              return <option key={index} value={co}>{co}</option>
            })}
          </select>
        </div>
        {/* colors */}
          ...
          <div className="colors">
            {
              colors.map((col,index)=>{
                ...
                return (
                  ...
                  data-color={col}
                  onClick={updateFilters}>{color===col?<FaCheck/>:null}</button>
                )
              })
            }
          </div>
